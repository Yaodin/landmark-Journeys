...
The provenance block in every path records the method, limitations, institutional sources, Horizons query URL when applicable, and SHA-256 of the returned Horizons text. `sample_origin_ranges` gives inclusive index ranges labeled `modeled_reconstruction`, `modeled_launch_or_gap`, or `jpl_horizons_vectors`. The label identifies the source of a plotted sample, not its positional accuracy: even JPL's historical vectors may be reconstructed rather than direct telemetry. `--annotate-sample-origins-only` refreshes these labels in existing path files without resampling or changing positions.
...
- The early Earth-orbit reconstructions now place their **modeled** orbital perigee at insertion rather than at the launch-pad phase. Published achieved perigee/apogee replace a single assumed altitude for Vostok 1 ([NASA Vostok table](https://ntrs.nasa.gov/api/citations/19900009927/downloads/19900009927.pdf), 181/327 km), Friendship 7 ([NASA mission facts](https://www.nasa.gov/mission/mercury-atlas-6-friendship-7/), about 161/261 km), and STS-1 ([NASA 1981 launch record](https://ntrs.nasa.gov/api/citations/19920074895/downloads/19920074895.pdf), 238/250 km). These dimensions are sourced; the plotted orbital-plane phase, ascent, and entry states remain reconstructions.
- Shenzhou 5 starts on [CMSA's achieved 199.14 by 347.8 km ellipse](https://en.cmse.gov.cn/missions/shenzhouv/) and transitions on the fifth circuit to the published 343 km circular orbit. A [contemporary People’s Daily account](https://cn.govopendata.com/renminribao/2003/10/16/2/) places the burn at 15:54 Beijing time (07:54 UTC). Its apogee phase at the burn is still modeled; this is not a recovered state history.
...
- Apollo 8's first two **modeled** lunar revolutions now use the [NASA mission report's](https://www.nasa.gov/wp-content/uploads/static/history/alsj/a410/A08_MissionReport.pdf) 60.0 by 168.5-nautical-mile initial ellipse, transitioning at its 73:35:07 mission-elapsed-time circularization to approximately 60 nmi for the remaining eight revolutions. This replaces a wholly circular depiction, but the orbit plane and samples are still illustrative. Rebuilt lunar paths record the JPL Moon-body query URL and response SHA-256 under `provenance.supplemental_queries`, distinct from spacecraft ephemerides.
...
- Mariner 4, Venera 7, and Viking 1: heliocentric transfer reconstructions constrained by JPL Earth/target-planet ephemerides. Their paths record both body-ephemeris query URLs and response SHA-256 hashes as supplemental sources; they do **not** contain recovered spacecraft vectors. [Viking's navigation report](https://ntrs.nasa.gov/api/citations/19800012912/downloads/19800012912.pdf) identifies correction dates, but its overview timeline is not a sampled state history and has not been turned into invented fixes.
